        ...
        <el-row :gutter="20">
            <el-col :span="24">
                        ...
                        <el-card shadow="hover" :body-style="{ padding: '0px' }">
                            <div class="grid-content grid-con-6">
                                ...
                                <div class="grid-cont-right">
                                    ...
                                    <div>临停收入(元)</div>
                                    <el-tooltip class="item" effect="dark" content="临停欠费金额(元)" placement="bottom">
                                        <el-link type="danger" href="javascript:;" @click="moneyLink()">欠费
                                            {{census.unpaidAmount}}元
                                        </el-link>
                                    </el-tooltip>
                                </div>
                            </div>
                    ...
                    <!--<el-col :span="6">
                        <el-card shadow="hover" :body-style="{ padding: '0px' }">
                            <div class="grid-content grid-con-4">
                                ...
                                <div class="grid-cont-right">
                                    <div class="grid-num">{{census.unpaidAmount}}</div>
                                    &lt;!&ndash;<div class="grid-bfb">&ndash;&gt;
                                    &lt;!&ndash;<i class="dangertext">{{census.bl}}</i>&ndash;&gt;
                                    &lt;!&ndash;</div>&ndash;&gt;
                                    <div>临停欠费金额(元)</div>
                                </div>
                            </div>
                ...
                <el-card shadow="hover">
                    <template #header>
                        <div class="clearfix">
                            <span>今日停车订单数排名(该页面以车辆停入时间统计停车订单收入金额)</span>
                        </div>
                    </template>
                    <div class="handle-box">
                        <div class="left-panel">
                            <el-form inline label-width="80" size="small" class="lineH0">
                                <el-form-item label="停车场名称" class="search-mb0">
                                    <el-input
                                            v-model="query.name"
                    ...
                    <el-table :show-header="true" height="380" :data="todoList" border class="table" ref="multipleTable"
                        ...
                        <!--</el-table-column>-->
                        <el-table-column label="停车场名称" prop="title" align="center">
...
<script>
    ...
    import {
        daysCurrentOrderTrends,
        daysMonthlyTrend,
        parkingOrderRanking,
        getOrderList,
        getMonthlyPaymentData,
        exportParkingOrderRanking,
        parkingStatistics,
        exportBy,
    } from "../../../api/index";
    ...
    export default {
        ...
        setup() {
            ...
            const getStartTime = () => {
                ...
                query.startTime = now.getFullYear().toString() + "-" + month.toString() + "-" + date.toString();
            ...
            //日期控件change方法
            const getQueryDate = () => {
                if (null == query.time || [] == query.time || "" == query.time) {
                    query.startTime = "";
                    query.endTime = "";
                } else {
                    // console.log(dataFormat(query.time[0]));
                    query.startTime = dataFormat(query.time[0]);
                    query.endTime = dataFormat(query.time[1]);
                }
            ...
            const exportSearch = () => {
                ...
                exportParkingOrderRanking(query).then((res) => {
                    const url = window.URL.createObjectURL(new Blob([res]));
                    const link = document.createElement("a");
                    link.href = url;
                    link.setAttribute("download", "停车场概览.xls");
            ...
            //     parkingStatistics(query).then((res) => {
            //         console.log(res)
            //         census.mpCount = res.data.mpCount;
            //         census.sumCost = res.data.sumCost;
            //         census.soCount = res.data.soCount;
            //         census.sumAmount = res.data.sumAmount;
            //         census.bl = res.data.bl;
            //     });
            // };
            //今日停车订单数排名
            const getParkingOrderRanking = () => {
                parkingOrderRanking(query).then((res) => {
                    const list = [];
            ...
            //近十天包月趋势
            const tenDaysMonthlyTrend = () => {
                daysMonthlyTrend(query).then((res) => {
                    options3.xAxis[0].data = res.data.dates;
                    options3.series[0].data = res.data.monthlyCount;
                    options3.series[1].data = res.data.monthlyIncome;
            ...
            //流动车近十天趋势
            const tenDaysCurrentOrderTrends = () => {
                daysCurrentOrderTrends(query).then((res) => {
                    options4.xAxis[0].data = res.data.dates;
                    options4.series[0].data = res.data.orderCount;
                    options4.series[1].data = res.data.receivable;
                    options4.series[2].data = res.data.actual;
            ...
            const handleView = (index, row, type) => {
                ...
                orderQuery.parkId = row.id;
                orderQuery.pageType = type;
                // 订单表格数据
                getOrderList(orderQuery).then((res) => {
        ...
        methods: {
            moneyLink() {
                ...
                this.$router.push({
                    path: '/arrearsstatic',
                    query: {menuType: "1", startTime: this.query.startTime, endTime: this.query.endTime},
                })
            },
            towNumber(val) {
