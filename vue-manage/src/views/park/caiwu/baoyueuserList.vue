    ...
    <div>
        <div class="container">
            <div class="handle-box">
                <div class="left-panel">
                    <el-button
                            ...
                            v-permission="'park_monthusermanaA_add'"
                    >添加
                    </el-button>

                    <el-button type="primary" size="small" icon="el-icon-document" @click="excelImport()"
                               v-permission="['park_monthusermanaA_add']">个人包月 Excel导入
                    </el-button>
                </div>
                <div class="right-panel">
                    ...
                    <el-button
                            type="success"
                            size="small"
                            @click="byexport(query.isTheCompany, 1)"
                            icon="el-icon-upload2"
                            v-permission="'park_monthusermanaA_excel'"
                    >导出
                    </el-button>
            ...
            >
                ...
                >
                    <template #default="scope">
                        <p v-if="scope.row.carType == 1">蓝牌</p>
                        <p v-if="scope.row.carType == 2">绿牌</p>
                        <p v-if="scope.row.carType == 3">黄牌</p>
                    </template>
                </el-table-column>


                <el-table-column prop="parkNameList" label="包月停车场" align="center">
                    <template #default="scope">
                        <el-tag
                                size="small"
                                v-for="(item, index) in scope.row.parkNameList"
                                :key="index"
                                class="mar5 mb5"
                        >{{ item }}
                        </el-tag
                ...
                >
                    <template #default="scope">
                        <p v-if="scope.row.paymentType == 1">包月</p>
                        <p v-if="scope.row.paymentType == 2">微信</p>
                        <p v-if="scope.row.paymentType == 3">支付宝</p>
                        <p v-if="scope.row.paymentType == 4">钱包</p>
                        <p v-if="scope.row.paymentType == 5">现金</p>
                        <p v-if="scope.row.paymentType == 6">银行卡</p>
                        <p v-if="scope.row.paymentType == 7">商家支付</p>
                        <p v-if="scope.row.paymentType == 8">聚合支付</p>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="userName"
                ...
                <el-table-column label="操作" width="220" align="center">
                    <template #default="scope">
                        ...
                        <el-button
                                ...
                                v-permission="'park_monthusermanaA_xf'"
                        >续费
                        </el-button>
                        <el-button
                                size="mini"
                                ...
                                v-permission="'park_monthusermanaA_edit'"
                        >编辑
                        </el-button>
                        <el-button
                                size="mini"
                                ...
                                v-permission="'park_monthusermanaA_delete'"
                        >删除
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <!--白名单模板下载上传-->
        <el-dialog width="650px" title="个人包月模板" v-model="viewVisibleExcel" :close-on-click-modal="false">
            <el-form label-width="130px" class="excel-form">
                <el-form-item label="个人包月模板下载">
                    <el-button type="success" size="small" icon="el-icon-download" @click="download()">Excel 下载
                    </el-button>
                </el-form-item>
                <el-form-item label="个人包月批量导入">
                    <input type="file" id="inputFile" @change="exportImport()" hidden/>
                    <el-button size="small" ref="inputFile" @click="inputFileClick()">Excel导入</el-button>
                </el-form-item>
            </el-form>
            <template #footer>
            </template>
            ...
            <el-form label-width="70px" size="small">
                ...
                <table class="desctable w mgb20" v-show="true">
                    <tr>
                        ...
                        <td class="tit" align="center">停车场</td>
                    </tr>
                    <tr>
        ...
        <el-dialog :title="dialogT" v-model="infoVisible" width="40%" top="5vh">
            <table class="desctable mgb20 w">
                ...
                <tr>
                    <td class="tit" width="160">包月停车场</td>
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            const download = () => {
                downloadMonthly(reactive({})).then((res) => {
                    const url = window.URL.createObjectURL(new Blob([res]));
                    const link = document.createElement("a");
                    link.href = url;
                    link.setAttribute("download", "个人包月模板.xlsx");
        ...
        methods: {
            ...
            getComCars() {
                var that = this;
                console.log("that.form.companyId", that.form.companyId)
                if (that.form.companyId != null && that.form.companyId != "") {
            ...
            saveEdit() {
                ...
                if (dstr.search("GMT") != -1) {
                    ...
                    // that.form.bean.start_time = Year + "-" + month + "-" + day + " " + hours + ":" + minutes + ":" + seconds;
                    that.form.bean.start_time = Year + "-" + month + "-" + day;
                }
            ...
            exportImport() {
                ...
                axios({
                    url: File_URL.file_down + "monthlymanagement/importMonthlyList",
                    method: "POST",
                    data: formData,
                    headers: {
