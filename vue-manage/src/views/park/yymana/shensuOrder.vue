    ...
    <div>
        <div class="container">
            <div class="handle-box f-cb">
                <div class="left-panel">
                    ...
                    >申诉总数：{{ listCount.sumData }}
                    </el-button
                    >
                    <el-button size="small" type="danger" plain
                    >已处理：{{ listCount.yclData }}
                    </el-button
                    >
                    <el-button size="small" type="danger" plain
                    >未处理：{{ listCount.wclData }}
                    </el-button
                    >
                </div>
                <div class="right-panel">
                    <el-form inline size="small">
                        ...
                        <el-form-item class="mb0">
                            ...
                            <el-button size="small" icon="el-icon-upload2" type="success" @click="exportshensu"
                                       v-permission="'park_shensuorderA_excel'"
                            >导出
                            </el-button>
                        </el-form-item>
                    </el-form>
                ...
                <el-table-column label="操作" width="120" align="center">
                    <template #default="scope">
                        <el-button
                                ...
                                v-permission="'park_shensuorderA_details'"
                        >查看
                        </el-button>
                    </template>
                </el-table-column>
                ...
                >
                    ...
                    <table class="desctable mgb20 w">
                        <template v-for="(item, i) in detailsData.appeals" :key="i">
                            ...
                            <tr>
                                <td class="tit" width="60">是否跨月</td>
                                <td colspan="3">
                                    <el-tag type="danger" v-if="item.is_ky == '1'">是</el-tag>
                                    <el-tag type="success" v-if="item.is_ky == '0'">否</el-tag>
                                </td>
                            </tr>
                        </template>
        ...
        <span class="dialog-footer">
          ...
          <el-button
                  type="danger"
                  v-show="shTypehide"
                  @click="parkApproveData(detailsData.records, '2')"
                  :disabled="isDisabl"
          >通过</el-button
          >
          <el-button
                  v-show="shTypehide"
                  @click="parkApproveData(detailsData.records, '3')"
                  :disabled="isDisabl"
          >驳回</el-button
          >
          <el-button
                  type="danger"
                  @click="handleView(detailsData.records)"
                  v-if="shhide"
          >处理申诉</el-button
          >
          <el-button type="info" v-show="tzxjfkhide" @click="noticeFeedback"
          >通知收费员反馈</el-button
          >
        </span>
            </template>
        ...
        <span class="dialog-footer">
          ...
          <el-button type="danger" v-if="qdbtn" @click="saveEdit" :disabled="isDisabl"
          >确定</el-button
          >
        </span>
            </template>
        ...
        <!-- 通知巡检员反馈 -->
        <el-dialog title="通知收费员反馈" v-model="appealVisible" width="40%">
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                operateappealPageData(query).then((res) => {
                    ...
                    //申述总数
                    listCount.value.sumData = res.data.zs_count;
                    //已处理
                    listCount.value.yclData = res.data.ycl_count;
                    //未处理
            ...
            const shhide = ref(true);
            const shTypehide = ref(false);
            const tzxjfkhide = ref(false);
            const fileList2 = ref([]);
            const fileIds = ref([]);
            const detailsData = ref({});
            const feedbacks = ref({});
            const qdbtn = ref(true);
            let form = ref({
                end_time: "",
                reject_reason: "",
                handle_type: 1,
            ...
            const handleEdit = (index, row) => {
                ...
                operateappealDetailsData(
                    ...
                    .then((res) => {
                        ...
                        //判断申诉时间是否跨月
                        if (null != detailsData.value.appeals) {
                            var appeals = detailsData.value.appeals;
                            for (var i = 0; i < appeals.length; i++) {
                                var appeal_time = appeals[i].appeal_time;
                                var dt1 = new Date(appeal_time.replace(/-/g, "/"));
                                var dt2 = new Date();
                                if (
                                    dt1.getFullYear() == dt2.getFullYear() &&
                                    dt1.getMonth() == dt2.getMonth()
                                ) {
                                    detailsData.value.appeals[i].is_ky = 0;
                                } else {
                                    detailsData.value.appeals[i].is_ky = 1;
                                }
                            }
                        }
                        //停车场id
                        let parking_order_id = row.parking_order_id;
                        if (null != detailsData.value.records) {
                            shhide.value = false;
                        } else {
                            shhide.value = true;
                            tzxjfkhide.value = true;
                        }
                        if (res.userType == 0) {
                            if (null != detailsData.value.records) {
                                if (detailsData.value.records.approve_status == "1") {
                                    shTypehide.value = true;
                                    tzxjfkhide.value = true;
                                } else {
                                    shTypehide.value = false;
                                    tzxjfkhide.value = false;
                                }
                            } else {
                                shTypehide.value = false;
                                tzxjfkhide.value = true;
            ...
            const handleView = (row) => {
                ...
                upApprove(reactive({id: form.value.operate_appeal_id})).then((res) => {
                    if (res.code == -1) {
                        ElMessage.error(res.msg);
                        qdbtn.value = false;
                    } else {
            ...
            //日期控件change方法
            const getQueryDate1 = () => {
                console.log(dateFormat(timeData.value.time1[0]));
                query.startTime = dateFormat(timeData.value.time1[0]);
                console.log(query.startTime);
                query.endTime = dateFormat(timeData.value.time1[1]);
            };
        ...
        methods: {
            ...
            saveAppeal(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        let msg = "";
                        let query = reactive({
                            operate_appeal_id: _that.form.operate_appeal_id,
            ...
            parkApproveData(row, status) {
                var _this = this;
                _this.isDisabl = true;
                if (row.id == null || row.id == undefined || row.id == "") {
                    _this.isDisabl = false;
                    ElMessage.error("请先处理申诉！");
                } else {
                    var flag = 0;
                    diversePattern(reactive({id: row.id, approve_status: status}))
                        .then((res) => {
                            flag = res.code;
                        })
                        .then(() => {
            ...
            exportshensu() {
                ...
                exportshensuData(this.query).then((res) => {
                    ...
                    link.href = url;
                    link.setAttribute("download", "停车申诉订单.xls");
