        ...
        <el-card shadow="hover">
          ...
          <el-form label-width="70px" size="small">
            <el-form-item label="路内名称">
              <el-input v-model="result.road.road_name"></el-input>
            </el-form-item>
            <el-form-item label="路内等级">
              <el-select
                v-model="result.road.road_level"
                filterable
                size="small"
                placeholder="所有区域"
                class="w"
      ...
      <el-col :span="12">
        <el-card shadow="hover" class="h500">
          ...
          <el-form label-width="150px" label-position="left">
            <el-form-item label="选择收费方案(蓝)">
              <el-select
                v-model="result.road.blue_charge_id"
                filterable
                size="small"
                placeholder="所有区域"
                class="w"
            ...
            <el-form-item label="选择收费方案(绿牌)">
              <el-select
                v-model="result.road.green_charge_id"
                filterable
                size="small"
                placeholder="所有区域"
                class="w"
            ...
            <el-form-item label="选择收费方案(黄牌)">
              <el-select
                v-model="result.road.yellow_charge_id"
                filterable
                size="small"
                placeholder="所有区域"
                class="w"
      ...
      <el-col :span="24">
        <el-card shadow="hover">
          ...
          <el-form label-width="70px" size="small">
            ...
            <el-form-item label="坐标范围">
              <el-input v-model="result.road.longitude" placeholder=""></el-input>
              <span class="color999"
                >操作提示：http://lbs.qq.com/tool/getpoint/index.html，以腾讯位置服务坐标拾取为准。</span
              >
            </el-form-item>
...
import { getRoad,editRoad,queryAreaData,queryStreetData,chargeProgrammeData,dictData, editDevice } from "../../api/index";
...
export default {
  ...
  setup() {
    ...
    let result = reactive({
     ...
     // 获取表格数据 
    let road_id = reactive({
       id:router.currentRoute.value.query.id
    });
    const getData = () => {

      getRoad(road_id).then((res) => {
        console.log("sss");
        console.log(res.data);
        result.road = res.data;
      });
    };
    getData();
