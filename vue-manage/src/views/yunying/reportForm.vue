  ...
  <div>
    <div class="container">
      <el-tabs v-model="activeTab" @tab-click="handleTab">
        ...
        <el-tab-pane label="月租车卡统计" name="yzkDetail">
          <div class="handle-box mt20">
            ...
            <div class="right-panel">
              <div class="amount-box line-des">
                <h6>总计</h6>
                <!--<div class="amount-des">-->
                  <!--<p>-->
                    <!--车次：<span>{{ yzkStatistics.frequency }}</span-->
                    <!--&gt;(次)-->
                  <!--</p>-->
                <!--</div>-->
                <div class="amount-des">
                  <p>
                    车辆：<span>{{ yzkStatistics.carNumAll }}</span
                    >(辆)
                  </p>
                </div>
                <div class="amount-des">
                  <p>
                    金额：<span>{{ yzkStatistics.amount }}</span
                    >(元)
                  </p>
                </div>
                <div class="clear"></div>
          ...
          <el-table :data="yzkList" border style="width: 100%">
            <el-table-column
              prop="style"
              label="办理方式"
              align="center"
        ...
        <el-tab-pane label="流动车支付流水" name="flowPay">
            ...
            <div class="right-panel">
              <div class="amount-box">
                ...
                <div class="amount-des">
                  ...
                  <p>
                    金额：<span v-text="result.orderZj.car_price"></span>(元)
                  </p>
                </div>
                <div class="clear"></div>
              </div>
          ...
          <div class="mt20"></div>
          <div class="small-title">
            <div class="table-title">流动车流水明细（第三方）</div>
            <div class="amount-samll-box">
              <b>合</b>
              <!--<p class="mr-20">-->
          ...
          <el-table :data="tableData3Other" border style="width: 100%">
            ...
            <el-table-column prop="pay_type" label="支付方式" align="center">
              <template #default="scope">
                <p v-if="scope.row.pay_type == '1'">包月支付</p>
                <p v-if="scope.row.pay_type == '4'">钱包支付</p>
                <p v-if="scope.row.pay_type == '8'">聚合支付</p>
              </template>
            </el-table-column>
            <el-table-column
              prop="pay_channel"
...
export default {
  ...
  setup() {
    ...
    const queryRoad = reactive({
      streetId: yszlQuery.street_id,
    });
    const getRoad = () => {
      queryRoad.streetId = yszlQuery.street_id;
      yszlQuery.variance = "";
      query.road_id = "";
    ...
    const getRoadSORList = () => {
      roadSORList(yszlQuery).then((res) => {
        var data = res.data;
    ...
    // 营收总览Excel
    const downSORExcel = () => {
      summaryORExcel(yszlQuery).then((res) => {
        console.log(res.data);
        console.log(document.createElement("a"));
        const url = window.URL.createObjectURL(new Blob([res]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "路内营收报表.xlsx");
        document.body.appendChild(link);
        link.click();
      });
    };
