...
import com.jsdc.zhtc.dao.AlPayConfigDao;
...
/**
 * 类 名: 阿里支付配置
 * 描 述: AlPayConfigService
 * 作 者: lw
 * 创 建：2021/12/30 14:55
 * 版 本：v1.0.0
 * 历 史: (版本) 作者 时间 注释
 */
@Service
@Transactional
public class AlPayConfigService extends BaseService<AlPayConfigDao, AlPayConfig> {
     ...
     */
    public ResultInfo selectAll(PageVo<AlPayConfig> data) {

        AlPayConfig bean = data.getBean();

        QueryWrapper<AlPayConfig> wrapper = new QueryWrapper<>();
        ...
        if (data.getPageNum() != null) {
            ...
            List<AlPayConfig> lists = selectList(wrapper);
            PageInfo<AlPayConfig> listPage = new PageInfo<>(lists);
            return ResultInfo.success(listPage);
        } else {
            List<AlPayConfig> lists = this.selectList(wrapper);
     ...
     *
     * @param
     * @return {@link AlPayConfig}
     */
    public AlPayConfig findAlPayConfig() {
        QueryWrapper<AlPayConfig> alWrapper = new QueryWrapper<>();
        alWrapper.eq("is_default", 0);
        AlPayConfig alPayConfig = this.selectOne(alWrapper);
        return alPayConfig;
    }
     ...
     * @return {@link ResultInfo}
     */
    public ResultInfo selectById(AlPayConfig bean) {
        AlPayConfig data = this.selectById(bean.getId());
        if (data != null) {
     ...
     * @return josn
     */
    public ResultInfo saveData(AlPayConfig bean) {
        ...
        if (bean.getIs_default() == 0) {
            UpdateWrapper<AlPayConfig> updateWrapper = new UpdateWrapper<>();
     ...
     * @return {@link ResultInfo}
     */
    public ResultInfo updateInfo(AlPayConfig bean) {

        //如要删除提前判断是否为默认 为默认则不允许删除
        if (bean.getIs_del() != null && !bean.getIs_del().equals(GlobalData.ISDEL_YES)) {
            AlPayConfig alPayConfig = this.selectById(bean.getId());
     ...
     * @return {@link ResultInfo}
     */
    public ResultInfo deleById(AlPayConfig bean) {
