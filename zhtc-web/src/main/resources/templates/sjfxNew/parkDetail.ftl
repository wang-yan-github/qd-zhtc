...
<div class="maincontainerNew maincontainerNewDetail">
    <div class="detail-left detail-left-park">
        <div class="detail-data-con">
            <div class="detail-data-inside">
                <div class="p-icon"></div>
                <div class="p-des">
                    <h4 id="name">建国西路停车场</h4>
            ...
            <div class="revenue-box">
                <div class="revenue-left">
                    ...
                    <div class="revenue-lf-text">
                        <h4 id="parking_today_orderCount">53,456</h4>
                        <p>
                            今停车次数
                            <span>昨停车次数：<span id="parking_yesterday_orderCount">30</span></span>
                        </p>
                    </div>
                </div>
                <div class="revenue-left">
                    ...
                    <div class="revenue-lf-text">
                        <h4 id="parking_ztCount">53,456</h4>
                        <p>
                            在停车辆数
                            <span>空余泊位数：<span id="parking_freeCount">30</span></span>
                        </p>
                    </div>
                </div>
            </div>
...
<script type="text/javascript" src="/sjfxNew/js/echart/pages/parkDetail.js"></script>
<#--<script type="text/javascript" src="/sjfxNew/js/countup.js"></script>-->
<script>
    var options = {
    ...
    function getParkViewIndexData() {
        $.ajax({
            ...
            success: function (res) {
                console.log("停车场详情 统计信息")
                console.log(res)

                $("#name").html(res.data.park_info.park_name);
                $("#code").html(res.data.park_info.park_code);

                // 地图
                maponLoad(res.data.park_info.longitude, res.data.park_info.latitude, 18);

                var pointJson = [];
                pointJson.push(res.data.park_info)
                console.log(pointJson)
                creatMarker(pointJson);

                // 今日/昨日停车次数、在停车辆数、空余泊位数
                var parking_ztCount = new CountUp("parking_ztCount", 0, res.data.parking_ztCount, 0, 2, options);
                var parking_freeCount = new CountUp("parking_freeCount", 0, res.data.parking_freeCount, 0, 2, options);
                var parking_today_orderCount = new CountUp("parking_today_orderCount", 0, res.data.parking_today_orderCount, 0, 2, options);
                var parking_yesterday_orderCount = new CountUp("parking_yesterday_orderCount", 0, res.data.parking_yesterday_orderCount, 0, 2, options);
                ...
                parking_ztCount.start();
                parking_freeCount.start();
                parking_today_orderCount.start();
                parking_yesterday_orderCount.start();
